...
def multiplication_chain(num):
    counter = 1  # Счётчик для записи сюда произведений чисел
    count_multy = 0  # Количество итераций
    while len(str(num)) > 1:  # Сравниваем длину строки с единицей
        for elem in str(num):
            counter *= int(elem)  # Записываем в счётчик произведение
        count_multy += 1  # Увеличиваем кол-во итераций
        num = counter  # Кладём получившееся произведение в num для последующего сравнения длины строки с единицей
        counter = 1  # Сбрасываем счётчик до 1
    return count_multy
