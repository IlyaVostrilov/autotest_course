...
def repeats(our_str):
    """
    Функция принимает на вход строку our_str,
    смотрит, сколько раз каждый символ уже встречался, и добавляет количество к символам с помощью постфикса формата _n
    """
    new_str = str()
    letters_dict = {}
    for letter in our_str:
        if letter not in letters_dict:  # Если буква встречается первый раз, то добавляем в счётчик единицу
            letters_dict[letter] = 1
        else:
            letters_dict[letter] += 1  # Если буква встречается не первый раз, то добавляем увеличиваем счётчик на
# единицу
