...
def everything_for_your_cat(cats_data):
    """
    Функция принимает неупорядоченный список кортежей и возвращает данные в формате:
    Имя_покупателя Фамилия_покупателя: Кличка_котика1, Возраст; ...; Кличка_котикаN, Возраст\n
    """
    info = {}  # Словарь для информации. Ключ - фамилия, имя. Значение - список из кличек и возраста котов
    our_str = str()
    for cat, age, name, surname in cats_data:
        fio = name + ' ' + surname  # Ключ словаря в формате "Имя Фамилия"
        kitty = list()  # Список для записи туда информации по каждому коту
        kitty.append(cat)  # Кличка кота добавляется в словарь
        kitty.append(age)  # Возраст кота добавляется в словарь
        if fio not in info:  # Проверяем, встречался ли уже данный владелец
            info[fio] = kitty  # Владелец не вcтречался - добавляем ему кота
        else:
            info[fio] += kitty  # Владелец встречался ранее, добавляем кота записанному ранее владельцу
            """
            Выводим полученную информацию в требуемом формате
            """
    for key, value in info.items():
        our_str += key + ': '
        n = 1  # итератор для сравнения с длиной списка кошек
        for animals in value:
            if n == len(value):
                our_str += f'{animals}\n'
                n = 1
            elif n % 2 != 0:
