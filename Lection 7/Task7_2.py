...
class PersonInfo:
    ...
    def path_deps(self):
        ...
        for path in self.path:
            if n != len(self.path) - 1:
                new_path = str(new_path) + str(self.path[n]) + ' --> '
                n += 1
            else:
                new_path = str(new_path) + str(self.path[n])
                return new_path
    ...
    def new_salary(self):
        """
        Возвращает новую зарплату, которая вычисляется по формуле:
        1337*Возраст*суммарное кол-во вхождений трех наиболее часто встречающихся букв из списка подразделений
        (регистр имеет значение "А" и "а" - разные буквы)
        """
        # Кладём названия подразделений в одну строку
        str_path = ''
        for podr in self.path:
            str_path += podr
        # Собираем словарь, где ключ - буква, а значение - кол-во вхождений буквы в строку
        letter_dict = {}
        for letter in str_path:
            if letter not in letter_dict:
                letter_dict[letter] = 1
            else:
                letter_dict[letter] += 1
        # Отсортируем наш словарь по значениям
        sorted_tuples = sorted(letter_dict.items(), key=operator.itemgetter(1))
        # Суммируем кол-во вхождений трех наиболее часто встречающихся букв из списка подразделений
        sum_of_numbers = sorted_tuples[-1][-1] + sorted_tuples[-2][-1] + sorted_tuples[-3][-1]
        return 1337 * self.age * sum_of_numbers
